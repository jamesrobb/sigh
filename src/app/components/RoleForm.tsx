...
export default function RoleForm({
  ...
  const [descriptionDocument, setDescriptionDocument] = useState<File | null>(
    null
  );
  ...
  const descriptionDocumentRef = useRef<HTMLInputElement | null>(null);
  ...
  useEffect(() => {
    ...
    fetch("/api/companies")
      ...
      .then((data) => {
        ...
        const nextCompanies = (
          data.companies as Array<{ id: number; name: string; url?: string | null }>
        )
          .map((entry) => ({
            id: Number(entry.id),
  ...
  async function uploadDescriptionDocument(roleId: number, file: File) {
    ...
    const data = await response.json().catch(() => null);
    if (!response.ok) {
      throw new Error(data?.error ?? "Failed to upload description document.");
    }
  }
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedCompany && !selectedCompanyId) {
    ...
    const normalizedLookup = normalizedCompany.toLowerCase();
    const matchedCompany = normalizedLookup
      ? companies.find((entry) => entry.name.toLowerCase() === normalizedLookup)
      : null;
    const resolvedCompanyId = selectedCompanyId ?? matchedCompany?.id ?? null;
    ...
    try {
      ...
      if (descriptionDocumentRef.current) {
        descriptionDocumentRef.current.value = "";
      }

      if (Number.isInteger(newId) && newId > 0) {
        if (descriptionDocument) {
          try {
            await uploadDescriptionDocument(newId, descriptionDocument);
          } catch (uploadError) {
            window.alert(
              uploadError instanceof Error
                ? uploadError.message
                : "Failed to upload description document."
            );
          }
        }
        router.push(`/roles/${newId}`);
      } else {
        router.refresh();
      }
    } catch (err) {
  ...
  const wrapperClassName =
    variant === "plain"
      ? "space-y-4"
      : "space-y-4 rounded-xl border border-[color:var(--border)] bg-[color:var(--card)] p-6";
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={4}
            className="w-full resize-none rounded-lg border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-2 text-sm text-[color:var(--foreground)] shadow-none outline-none"
          />
        </div>
        ...
        <div className="space-y-2">
          ...
          <input
            ref={descriptionDocumentRef}
            type="file"
            onChange={(event) =>
              setDescriptionDocument(event.target.files?.[0] ?? null)
            }
            className="w-full rounded-lg border border-[color:var(--border)] bg-[color:var(--card)] px-3 py-2 text-sm text-[color:var(--foreground)] shadow-none outline-none file:mr-3 file:rounded-md file:border file:border-[color:var(--border)] file:bg-[color:var(--accent)]/15 file:px-3 file:py-1.5 file:text-sm file:font-semibold file:text-[color:var(--foreground)] file:shadow-none file:transition file:hover:bg-[color:var(--accent)]/25 hover:border-[color:var(--accent)]/60"
          />
          {descriptionDocument ? (
            <p className="text-xs text-[color:var(--muted)]">
              Selected: {descriptionDocument.name}
            </p>
          ) : null}
        </div>
